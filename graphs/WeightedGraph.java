...
public class WeightedGraph<T> implements WeightedGraphInterface<T>
{
  ...
  private int numVertices; //size of graph
  private int maxVertices;
  private T[] vertices; //array of vertices --> that first array
  private int[][] edges; // two-d array; adjacency matrix
  private boolean[] marks;  // marks[i] is mark for vertices[i] // a 1D array 
  ...
  {
	//loop through vertices array and check if vertex in there
	  for (int i=0; i<numVertices; i++)
	  {
  ...
  {
	  //similar to hasVertex, loop through vertices array if vertices[i] = to vertex 
	  //if vertex in array then return i (The index)
	  for (int i=0; i<numVertices; i++)
	  {
		  if(vertices[i].equals(vertex)){
			  return i;
	  ...
	  return -1; //if vertex not in array
  }

  public void addEdge(T fromVertex, T toVertex, int weight) //row to column
  // Adds an edge with the specified weight from fromVertex to toVertex.
  {
   //addEdge("Philly", "Denver", 2000)
  ...
  public Queue<T> getToVertices(T vertex) //get adjacent vertices
